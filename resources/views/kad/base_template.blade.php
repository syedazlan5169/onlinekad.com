...
<head>
    ...
    <!-- Variable -->
    @php $colorCode = $design->color_code; $colorFooter = $design->color_footer; $primaryTextColor = $design->primaryTextColor; $secondaryTextColor = $design->secondary_text_color; @endphp
...
<body x-data="{ form_ucapan: false, form_rsvp: false, location_modal: false, reminder_modal: false, contact_modal: false, gift_modal: false}">
    ...
    <div x-data="{ show: true }" x-init="setTimeout(() => show = false, 5000)" x-show="show" x-transition:enter="transform ease-out duration-300 transition" x-transition:enter-start="translate-y-2 opacity-0 sm:translate-y-0 sm:translate-x-2" x-transition:enter-end="translate-y-0 opacity-100 sm:translate-x-0"
    x-transition:leave="transition ease-in duration-100" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" aria-live="assertive" class="pointer-events-none fixed inset-0 flex items-start px-4 py-6 sm:items-start sm:p-6">
    ...
    <div x-data="{ showModal: @if(!session('success')) true @else false @endif }" class="h-full w-full bg-cover bg-center lg:w-[400px] sm:bg-contain sm:mx-auto sm:max-w-lg sm:rounded-lg sm:shadow-xl" style="background-image: url('{{ asset($design->design_url_2) }}'); background-attachment: fixed; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.5);">
        ...
        <div class="h-screen w-full bg-cover bg-center" style="background-image: url('{{ asset($design->design_url_1) }}');">
            <div class="absolute inset-0 bg-white bg-opacity-20">
                <div x-show="showModal == false" class="flex flex-col justify-center gap-16 items-center h-full">
                    <h1 class="text-2xl text-center text-gray-600 animate-zoom-in" style="font-family: 'Noticia Text', cursive; color: {{ $primaryTextColor }};">{{ $kadData->tajuk_kad }}</h1> @if ($kadData->dua_pasangan_is_on == true)
        ...
        <div class="mb-12 pb-8">
            <div class="relative top-0 bg-white bg-opacity-40">
                ...
                <div x-data="{ isPlaying: true, audio: null }" x-init="audio = $refs.audioElement" class="flex items-center justify-center mb-4">
                    ...
                    <!-- Play/Pause Button -->
                    <button @click="isPlaying ? audio.pause() : audio.play(); isPlaying = !isPlaying" class="bg-gray-50 opacity-70 fixed top-2 z-50 flex items-center  space-x-2 py-0 px-2 rounded-full transition-colors duration-300 ease-in-out shadow-md">
                    ...
                    <div class="flex justify-center items-center">
                        <div x-show="showModal" x-cloak x-transition:leave="transition ease-out duration-[1500ms] transform" x-transition:leave-start="translate-y-0" x-transition:leave-end="-translate-y-full" class="fixed inset-0 z-[9999] flex items-center justify-center bg-opacity-75 bg-cover lg:w-[400px] lg:mx-auto lg:my-auto lg:flex lg:items-center lg:justify-center">
                            <!-- Modal Content -->
                            <div class="h-full w-full relative">
                                <!-- Blurred Background Image -->
                                <div class="absolute inset-0 bg-cover bg-center blur-sm" style="background-image: url('{{ asset($design->design_url_1) }}');">
                                </div>
                                ...
                                <div class="relative z-10 flex flex-col gap-6 items-center justify-center h-full">
                                    ...
                                    @endif
                                    <button @click="audio.play(); showModal = false" class="bg-blue-500 text-white py-2 px-4 rounded-lg shadow animate-pulse hover:bg-blue-600 transition">
                ...
                <div class="flex flex-col justify-center gap-5 items-center h-full">
                    ...
                    <di data-aos="zoom-in" data-aos-duration="2000" v>
                        ...
                        <p class="text-l font-bold text-center text-gray-600 font-sans px-14">{!! nl2br(e($kadData->alamat_majlis)) !!}</p>
                </div>
                ...
                <div class="mt-4" data-aos="zoom-in" data-aos-duration="2000" v>
                    ...
                    <ul class="space-y-2">
                        @php $firstEntry = collect($kadData->aturcara_majlis)->first(); @endphp @if(!empty($firstEntry['masa_acara']) || !empty($firstEntry['acara']))
                        <!-- Centered Icon -->
                        <div class="flex justify-center">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="{{ $colorCode }}" class="size-12">
                        ...
                        @endif @foreach($kadData->aturcara_majlis as $entry)
                ...
                @endif @if (!empty($kadData->info_tambahan))
                ...
                <div class="flex flex-col justify-center gap-5 items-center h-full px-6 pb-0 mb-0 ">
                    ...
                    <div class="items-center w-full h-auto mb-6 rounded-xl p-4 " style="background-color: {{ $colorCode }}; ">
                        ...
                        <script>
                            ...
                            const targetDate = new Date("{{ \Carbon\Carbon::parse($kadData->tarikh_majlis)->format('Y-m-d') }} {{ $kadData->masa_mula_majlis }}").getTime(); // Update the countdown every second const countdown = setInterval(() => { // Get the current date and time const now = new Date().getTime(); // Calculate the remaining
            time const distance = targetDate - now; // Time calculations for days, hours, minutes, and seconds const days = Math.floor(distance / (1000 * 60 * 60 * 24)); const hours = Math.floor((distance % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
            const minutes = Math.floor((distance % (1000 * 60 * 60)) / (1000 * 60)); const seconds = Math.floor((distance % (1000 * 60)) / 1000); // Update the countdown display document.getElementById("days").innerHTML = days; document.getElementById("hours").innerHTML
            = hours; document.getElementById("minutes").innerHTML = minutes; document.getElementById("seconds").innerHTML = seconds; // If the countdown is over, stop the timer and show a message if (distance
            < 0) { clearInterval(countdown); document.getElementById(
            "countdown").innerHTML=` <span class="text-2xl font-bold text-white">Majlis Sudah Berlangsung</span>`; } }, 1000);
                </script>
        </div>

        <!-- Slider -->
        <div class="main-slider size-80 w-full max-h-[400px] mx-auto overflow-hidden rounded-xl">
            @foreach($imageUrls as $url) @if (!empty($url))
    ...
    <img class="w-full px-3 h-18 mb-16 pt-0 mt-0" src="/images/Curly-Border-Bottom.png" alt=""> @endif
    ...
    <footer class="w-full mx-auto lg:w-[400px] flex justify-center items-center]">
        <div class="fixed bottom-0 z-50 w-full lg:w-[400px] mx-auto h-18 border-t" style="background-color: {{ $colorFooter }};">
            <div class="flex justify-center items-center my-1 w-[97%] gap-2 max-w-lg mx-auto font-medium">
                ...
                <h1 @click="gift_modal = true" class="text-4xl outline-white" style="color: {{ $colorCode }}; text-shadow: 1px 1px 0 #fff, -1px 1px 0 #fff, 1px -1px 0 #fff, -1px -1px 0 #fff;"><i class="fa-solid fa fa-gift"></i></h1> @endif @if ($kadData->package_id == 2 || $kadData->package_id == 3)
                ...
                @endif @if ($kadData->package_id == 2 || $kadData->package_id == 3)
    ...
    <form action="{{ route('tulis-ucapan') }}" method="POST">
        ...
        <div x-cloak x-show="form_ucapan" @click.away="form_ucapan = false" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100"
        x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <form action="{{ route('create-rsvp') }}" method="POST">
        @csrf
        <input type="hidden" name="kad_id" value="{{ $kadData->id }}">
        <div x-cloak x-show="form_rsvp" @click.away="form_rsvp = false" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100"
        x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="fixed inset-0 z-50 flex items-center justify-center p-4"
    ...
    <!-- Gift Modal Popup -->
    <div x-data="{ text: 'Copy', accountNumber: '{{ $kadData->account_number }}'} " x-cloak x-show="gift_modal" @click.away="gift_modal = false, text = 'Copy'" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
    ...
    class="fixed inset-0 z-50 flex items-center justify-center p-4" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            ...
            <div class="mt-5">
                <div class="bg-white mx-8 mb-5 p-5 rounded-lg">
                    ...
                    <div class="grid grid-cols-1 items-center pb-8">
                        ...
                        <label class="bg-gray-200 p-1 mt-1 text-xl flex items-center justify-center rounded-lg text-gray-700"><span x-text="accountNumber"></span>
                        </label>
                    </div>

                    <!-- Button -->
                    <div class="flex justify-center gap-3">
                        <x-primary-button class="h-8 w-24 flex justify-center items-center" @click="navigator.clipboard.writeText(accountNumber).then(() => text = 'Copied!' )"><span x-text="text"></span>
                        </x-primary-button>
    ...
    <div x-cloak x-show="location_modal" @click.away="location_modal = false" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100"
    x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="fixed inset-0 z-50 flex items-center justify-center p-4"
    aria-labelledby="modal-title" role="dialog" aria-modal="true">
    ...
    <div x-cloak x-show="reminder_modal" @click.away="reminder_modal = false" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100"
    x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="fixed inset-0 z-50 flex items-center justify-center p-4"
    aria-labelledby="modal-title" role="dialog" aria-modal="true">
    ...
    <div x-cloak x-show="contact_modal" @click.away="contact_modal = false" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100"
    x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="fixed inset-0 z-50 flex items-center justify-center p-4"
    aria-labelledby="modal-title" role="dialog" aria-modal="true">
        ...
        <div class="relative z-10 w-full max-w-md p-6 pb-0 rounded-xl mx-auto" style="background-color: {{ $colorCode }};">
            <div class="flex flex-col space-y-6 py-10">
                @foreach($kadData->nombor_telefon as $entry) @if(!empty($entry['nama']) && !empty($entry['nombor_telefon']))
